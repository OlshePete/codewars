...
const validRankList = [-8,-7,-6,-5,-4,-3,-2,-1,1,2,3,4,5,6,7,8]
class User {
    constructor(init=-8) {
        this.rank = init;
        this.progress = 0
    }
    incProgress(taskLevel){
        var task_progress = 0
        var range = validRankList.indexOf(taskLevel)-validRankList.indexOf(this.rank)
        switch (range) {
            case 0:
                task_progress = 3;
                break
            ...
            case 2:
            default:
                task_progress = 10 * range * range;
                break
        }
        this.updateProgress(task_progress)
    }
    checkAndUpdateRankByInt(count){
        const neededRank = validRankList[validRankList.indexOf(this.rank)+count]
        if (neededRank) {
            this.updateRank(neededRank)
        } else {
    ...
    updateRank(new_rank){
        this.rank = new_rank
        console.log('rank update', this.rank, this.progress);
    }
    updateProgress(new_progress) {
        var validatedProgress = this.progress + new_progress
        if ( validatedProgress>=100) {
            const levelCount = Math.floor(validatedProgress / 100) ?? 1
            this.checkAndUpdateRankByInt(levelCount)
            this.progress = this.progress + new_progress - (100 * levelCount)
        } else this.progress = validatedProgress

...
import { IFile, useSkifGlobal } from '@skif/utils';
...
} from '../../../../../../../kanban/utils/conferenceUtils';
...
import { usePanelCommentsContext } from '../../../../../../../SkifTaskPanel/context2/usePanelCommentsContext';
...
export const TaskConference: React.FC = () => {
  ...
  const { userId, conferences:conferences2, messagesAll:messagesAll2, messagesDefer:messagesDefer2, setDeferMessage } =
    usePanelDataContext();
  const {conferences, messagesAll, messagesDefer, setDeferMessage:setDeferMessage2 } = usePanelCommentsContext(); 
  const { task } = useTaskPanelContext();
console.log(`\x1b[36m context \x1b[0m`,`: `,{conferences, messagesAll, messagesDefer })
  ...
  const sendMessage = () => {
    ...
    setDeferMessage2({ ...newMessage, attributes: { sent: new Date().getTime() } } as ITaskMessage);
    // loadConference(backend, task?.task.id);
    setInputValue('');
    setFiles([]);
  };
  ...
  }, [conferences, backend, task?.task.id]);
